...
def stair_info_callback(msg):
    ...
    for x, z in xz_points[1:]:
        ...
        if dz >= min_step_height and dz <= max_step_height and dx >= min_step_depth:
            ...
            rospy.loginfo(f"Stair Detected: Distance={x:.2f} cm Height={dz:.2f} cm Angle={angle:.2f} degrees")
            ...
            if len(stair_info_callback.data_buffer) > 1:
                stair_info_callback.data_buffer.pop(0)

            # Calculate averages
            if len(stair_info_callback.data_buffer) == 1:
                avg_x = sum(item[0] for item in stair_info_callback.data_buffer) / 1
                avg_dz = sum(item[1] for item in stair_info_callback.data_buffer) / 1
                avg_angle = sum(item[2] for item in stair_info_callback.data_buffer) / 1

                stair_msg = StairInfo()
                stair_msg.distance = avg_x  / 10
                ...
                if (now - last_pub_time).to_sec() >= 3.0:  
